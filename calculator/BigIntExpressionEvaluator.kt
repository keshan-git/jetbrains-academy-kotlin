...
class BigIntExpressionEvaluator {
    private val memory = mutableMapOf<String, BigInteger>()
    private val debug = false

    fun evaluate(expressionString: String): BigInteger {
    ...
    private fun evaluate(expression: Queue<String>): BigInteger {
        if (expression.size == 1) {
            return BigInteger(expression.poll())
        }
     ...
     */
    private fun evaluatePostfix(postfixExpression: Queue<String>): BigInteger {
        val operands = Stack<BigInteger>();
    ...
    private fun convertToValue(operand: String): BigInteger {
        if (isNumber(operand)) {
    ...
    private fun calc(operandX: BigInteger, operandY: BigInteger, operator: String): BigInteger {
